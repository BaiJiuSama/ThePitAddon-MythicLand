...
class FreeCE : Runnable {
    ...
    private fun getRewardExp(prestige: Int): Double {
        ...
        var needExp = 0.2
        for (level in 1..120) needExp += LevelUtil.getLevelExpRequired(prestige, level)
    ...
    private fun autoPrestige(player: Player) {
        ...
        //FastPass Perk
        if (data != null) profile.experience = LevelUtil.getLevelTotalExperience(profile.getPrestige(), 50)
        ...
        if (profile.getPrestige() > 4) award += 10
        if (profile.getPrestige() > 10) award += 10
        if (profile.getPrestige() > 14) award += 10
        profile.renown += award
        CC.boardCast(
            MessageType.PRESTIGE,
            "&e&l精通! &7${RankUtil.getPlayerColoredName(player.name)} &7解锁了精通 &e${RomanUtil.convert(profile.getPrestige())} &7,gg!"
        )
