...
public class BadApple extends AbstractEnchantment implements  ITickTask, MovementHandler {
    ...
    @SneakyThrows
    public BadApple() {
        this.music = NBSDecoder.parse(ThePitAddon.getInstance().getClass().getClassLoader().getResourceAsStream("BadApple.nbs"));
        new BukkitRunnable() {
            @Override
            public void run() {
    ...
    public void handle(int enchantLevel, Player target) {
        SongUtil.songPlay(target, playerMap, music); //ez
    }
